...
Usage:
    ...
    python -m socialseed_e2e.mock_api
...
try:
    ...
    raise ImportError(
        "Flask is required for the mock server. Install with: pip install flask>=2.0.0"
    )
...
class MockAPIServer:
    ...
    def _register_routes(self) -> None:
        ...
        def list_users():
            ...
            users_response = [
                {k: v for k, v in u.items() if k != "password"} for u in paginated_users
            ]
        ...
        def delete_user(user_id: str):
            ...
            tokens_to_remove = [
                token for token, uid in self.auth_tokens.items() if uid == user_id
            ]
