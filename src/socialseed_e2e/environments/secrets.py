...
class VaultSecretProvider(SecretProvider):
    ...
    def __init__(self, url: str, token: str, mount_point: str = "secret"):
        try:
            ...
            raise ImportError(
                "hvac library is required for VaultSecretProvider. Install with pip install hvac"
            )
...
class AWSSecretManagerProvider(SecretProvider):
    ...
    def __init__(self, region_name: str = "us-east-1", profile_name: Optional[str] = None):
        try:
            ...
            raise ImportError(
                "boto3 library is required for AWSSecretManagerProvider. Install with pip install boto3"
            )
            ...
            if "SecretString" in get_secret_value_response:
                ...
                if json_key:
                    try:
                        ...
                        logger.warning(
                            f"Secret {secret_name} is not valid JSON, cannot extract {json_key}"
                        )
                        return None
                return secret
            return None
...
def get_secrets_provider(config: Dict[str, Any]) -> SecretProvider:
    ...
    providers.append(
        EnvironmentVariableSecretProvider(prefix=config.get("env_var_prefix", "SOCIALSEED_"))
    )
